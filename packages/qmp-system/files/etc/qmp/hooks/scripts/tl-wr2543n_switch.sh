...
echo "Configuring TP-Link TL-WR2543N(D) switch [INET|LAN|LAN|LAN|MESH]"
	...
	uci set network.@switch_vlan[0].ports="9t 1 2 3"
	uci set network.@switch_vlan[1]=switch_vlan
	uci set network.@switch_vlan[1].device=switch0
	uci set network.@switch_vlan[1].vlan=2
	uci set network.@switch_vlan[1].vid=2
	uci set network.@switch_vlan[1].ports="9t 0"
	...
	uci set network.@switch_vlan[2].ports="9t 4"
